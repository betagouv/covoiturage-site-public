...
export default function SelectPeriod(props:SelectPeriodProps) {
  ...
  return (
    <>
      <SelectInList label='Mois' id={props.month} list={monthList} sx={{  minWidth: 120 }} onChange={handlerChangeMonth} />
      <SelectInList label='Année' id={props.year} list={yearList} sx={{ ml: 3, minWidth: 120 }} onChange={handlerChangeYear} />
    </>
  );
}